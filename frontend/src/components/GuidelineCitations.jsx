...
function citationLabel(guideline) {
  const source = cleanCitationPart(guideline.source || guideline.title || 'Guideline')
  const section = cleanCitationPart(guideline.section || '')
  if (!section || source.toLowerCase().includes(section.toLowerCase())) return source
...
export default function GuidelineCitations({ guidelines = [] }) {
  ...
  if (!guidelines.length) {
    return <p className="mt-3 text-sm text-slate-500">No guidelines retrieved for this section</p>
  }
    ...
    <div className="mt-4">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {guidelines.map((guideline, index) => {
          const label = citationLabel(guideline)
            ...
            <div key={`${label}-${index}`} className="w-full sm:w-auto">
              ...
              {open && (
                <div className="mt-2 max-w-xl rounded-md border border-blue-200 bg-white p-3 text-sm text-slate-700 shadow-sm">
                  <div className="font-semibold text-slate-900">{cleanCitationPart(guideline.source || label)}</div>
                  <div className="mt-1 text-xs text-slate-500">
                    Year: {guideline.year || 'unknown'} | Section: {cleanCitationPart(guideline.section) || 'not specified'} | Relevance: {guideline.relevance_score ?? 'n/a'}
                  </div>
