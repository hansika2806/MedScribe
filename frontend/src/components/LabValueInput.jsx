...
import { getPendingLabRows } from '../utils/labs'

export default function LabValueInput({ sessionId, responseData, onSaved }) {
  const pendingLabs = useMemo(() => getPendingLabRows(responseData), [responseData])
  const [rows, setRows] = useState(
    pendingLabs.map((lab) => ({
      ...
      display_name: lab.display_name || lab.lab_name || lab.name || 'Lab value',
  ...
  return (
    <section className="rounded-lg border border-amber-300 bg-amber-50 p-5">
      ...
      <p className="mt-2 text-sm text-amber-900">
        These values are missing or uncertain. Verified PDF values are already accepted and do not need manual entry.
      </p>

      <div className="mt-4 space-y-3">
        {rows.map((row, index) => (
          <div key={`${row.lab_name}-${index}`} className="grid gap-3 rounded-md border border-amber-200 bg-white p-3 md:grid-cols-5">
            ...
            <label className="text-sm">
              ...
              <input value={row.reference_range} onChange={(event) => updateRow(index, 'reference_range', event.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
            </label>
            <label className="text-sm">
